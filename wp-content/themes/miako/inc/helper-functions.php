...
if ( !class_exists( 'RDTheme_Helper' ) ) {
	...
	class RDTheme_Helper {
		...
		public static function fonts_url(){
			$fonts_url = '';
			if ( 'off' !== _x( 'on', 'Google fonts - Open Sans and Raleway : on or off', 'miako' ) ) {
				$fonts_url = add_query_arg( 'family', urlencode( 'Open Sans:400,300,600,700,800|Raleway:400,300,500,600,700,800|Droid Sans:400,700' ), "//fonts.googleapis.com/css" );
			} 
			return $fonts_url;
		}
		...
		public static function nav_menu_args(){
			...
			if ( ( is_single() || is_page() ) ) {
				$rdtheme_menuid = get_post_meta( get_the_id(), 'miako_page_menu', true );
		...
		public static function dynamic_internal_style(){
			...
			$dynamic_css  = ob_get_clean();
			//$dynamic_css .= wp_kses_post( RDTheme::$options['custom_css'] ); // custom css
			wp_register_style( 'miako-dynamic', false );
			wp_enqueue_style( 'miako-dynamic' );
			wp_add_inline_style( 'miako-dynamic', $dynamic_css );	
		}
	}
}